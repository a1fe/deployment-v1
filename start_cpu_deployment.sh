...
log_info "⏳ Ожидание полной инициализации воркеров..."
sleep 10  # Даём больше времени на инициализацию
./start_celery.sh status

# Проверяем регистрацию задач
log_info "🔍 Проверка регистрации задач в воркерах..."
if celery -A celery_app.celery_app inspect registered >/dev/null 2>&1; then
    log_success "✅ Все задачи успешно зарегистрированы в воркерах"
else
    log_warning "⚠️ Возможны проблемы с регистрацией задач"
fi
...
# Проверка Flower
if curl -I "http://localhost:${FLOWER_PORT}" >/dev/null 2>&1; then
    log_success "Flower работает и отвечает на запросы"
else
    log_warning "Flower может быть еще не готов или недоступен"
fi

# Проверка Celery Workers
if celery -A celery_app.celery_app inspect active >/dev/null 2>&1; then
    log_success "Celery Workers активны и готовы к работе"
    
    # Тестовая задача для проверки
    log_info "🧪 Тестирование выполнения задач..."
    python -c "
from celery_app.celery_app import celery_app
result = celery_app.send_task('common.tasks.fillout_tasks.fetch_resume_data')
print(f'✅ Тестовая задача отправлена: {result.id}')
" && log_success "Задачи отправляются и принимаются воркерами" || log_warning "Проблема с отправкой задач"
else
